...
class RocketParameters:
    ...
    def get_thruster_force_direction(self, thruster: str) -> int:
        ...
        raise ValueError(
            f"Thruster must be one of {self.thruster_labels}"
        )
    ...
    def calc_angular_acc(self, thruster_forces: Tuple[float, float, float, float]) -> float:
        ...
        return sum((self.calc_thruster_angular_acc(t, f) for t, f in zip(self.thruster_labels, thruster_forces)))
    ...
    def calc_acc_relative2rocket(self, engine_forces: Tuple[float, float, float, float, float]) -> Coordinate:
        ...
        horizontal_acc = sum((self.get_thruster_force_direction(t) * self.calc_abs_acc(f) for t, f in zip(self.thruster_labels, thruster_forces)))
